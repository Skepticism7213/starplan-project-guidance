...
### Chat 模式（function calling）护栏测试

首次实测 `run_starplan_chat`（Qwen 自主工具调用编排）。结论分两面：

**机制可用**：Qwen 能正确按序调用工具并使用返回值，「百炼 function calling 完全不工作」的最坏担忧排除。

**发现幻觉并加护栏**：裸跑时 Qwen 编造了用户未提供的输入——日期编成 2024 年（错年份）、经纬度凭记忆猜成 36.45/117.13（真实值 36.49/117.18）、设备自选；最终总结还含大量无来源数值（气温、角距离、暗适应时间等）。已加三道护栏并重测：

| 护栏 | 措施 | 重测结果 |
|---|---|---|
| 输入·日期 | system prompt 注入当前日期 + 禁用过去年份 | 日期改为 2026 年，不再编错年份 |
| 输入·坐标 | 新增 `resolve_location` 工具 + 强制先查坐标 | 工具链变为 target_resolve→resolve_location→observability_plan，坐标 36.49/117.18 来自工具，坐标来源核查通过 |
| 输出·数值 | 最终总结幻觉核查（数值须逐字溯源工具输出）+ 坐标来源核查 | 准确标记 23 个无法溯源数值（Qwen 自行插值的逐时表格） |

**关键结论**：输入端编造可防（护栏已生效），但自由文本总结的无来源数值只能检测、不能干净清除。这正是生产入口采用 `run_starplan_nl` + `outreach_pack`（离散讲解要点，含不可溯源数字的要点整条删除＝预防）而非 chat 自由总结（仅检测）的原因。该对比是「为何需要确定性工具 + 幻觉防护」的有力实证。
...
| 3 | Qwen 工具调用编排模式 `run_starplan_chat`（function calling）实测：机制可用，但 Qwen 会编造输入（错年份日期、猜测经纬度）并在自由总结中插入无法溯源的数值 | runner.py / qwen_client.py | 已缓解：新增 `resolve_location` 工具 + system prompt 注入当前日期 + 最终总结幻觉核查与坐标来源核查；自由文本数值仅能检测不能清除，生产入口仍用 `run_starplan_nl` |
...
1. ~~测试 `run_starplan_chat`~~ ✅ 已完成：function calling 机制可用；已加三道护栏（`resolve_location` 工具 + 当前日期注入 + 幻觉/坐标核查），详见上文护栏测试小节。
...
6. 提交 chat 模式护栏改动（`qwen_client.py` + `runner.py`）与本报告更新，勿积压。
